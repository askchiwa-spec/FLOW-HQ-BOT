...
export default function SignInButton({ provider, callbackUrl = '/app/onboarding' }: SignInButtonProps) {
  ...
  if (provider.id === 'google') {
    // csrfToken is loaded async — disable the button until it's ready.
    // Submitting with an empty token causes NextAuth to reject the sign-in (OAuthSignin error).
    ...
    return (
      <form method="POST" action={`/api/auth/signin/${provider.id}`}>
        ...
        <button
          type="submit"
          disabled={!ready}
          className="w-full flex items-center justify-center gap-3 px-6 py-4 border border-white/10 rounded-xl text-white bg-white/5 hover:bg-white/10 hover:border-white/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-wait"
        >
          {ready ? (
            <>
            ...
            <span className="font-medium text-slate-400">Loading…</span>
          )}
        </button>
      </form>
    );
